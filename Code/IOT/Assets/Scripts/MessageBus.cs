...
public class MessageBus : MonoBehaviour
{
    ...
    private Dictionary<GameObject, float> objectOffsets = new Dictionary<GameObject, float>();
    ...
    public void SendMessageDelayed(MessageData message)
    {
        float distance = Vector3.Distance(message.sender.transform.position, message.receiver.transform.position);
        float delay = distance / SPEED_OF_LIGHT;  // Calculate delay based on speed of light
        double u1 = 1.0 - rand.NextDouble();
        double u2 = 1.0 - rand.NextDouble();
        double randStdNormal = Math.Sqrt(-2.0 * Math.Log(u1)) * Math.Sin(2.0 * Math.PI * u2);
        float antennaDelayMean = 0.0f; // Default values since parameters were removed
        float antennaDelayStd = 0.001f;
        float antennaDelay = (float)(randStdNormal * antennaDelayStd + antennaDelayMean);
    ...
    public float GetSimulatedTime(GameObject caller)
    {
        if (!objectOffsets.ContainsKey(caller)) 
        {
            objectOffsets[caller] = Random.Range(-0.5f, 0.5f);
        }
        return simulatedTime + objectOffsets[caller];
    }
}
